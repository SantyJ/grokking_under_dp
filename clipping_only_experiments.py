...
from clipping_only_utils import (evaluate, 
...
# For clipping, we need to use minibatches, so we don't force full batch size.
# The user should set a batch_size for clipping experiments.
if args.full_batch and not args.use_clipping:
...
if args.use_clipping:
    ...
    # Using make_private to apply clipping without noise
    model, optimizer, train_loader = privacy_engine.make_private(
        module=model,
        optimizer=optimizer,
        data_loader=train_loader,
        max_grad_norm=args.max_grad_norm,
        noise_multiplier=0.0,  # This is the key change for clipping-only
    )
    print(f"Using gradient clipping with max_grad_norm: {args.max_grad_norm}")
...
if args.full_batch and not args.use_clipping:
    ...
    # Clipping requires mini-batch training.
    pass
...
for epoch in range(args.num_epochs):
    ...
    if epoch % logger.log_frequency == 0:
        # No epsilon to print in clipping-only mode
